...
import { Ticket, Clock, Form } from "lucide-react";
import {
  Field,
  FieldGroup,
  FieldLabel,
  ...
  const checkSavedTicket = useCallback(async () => {
    ...
    try {
      ...
      if (state) {
        ...
        if (
          savedTicket > state.currentTicket &&
          savedTicket <= state.lastTicket
        ) {
  ...
  const handleGenerateTicket = async (e: React.SubmitEvent) => {
    ...
    try {
      const response = await fetch("/api/queue", {
        ...
        body: JSON.stringify({ action: "generate", name, email, tel }),
      });
          ...
          <Card className="w-full border-primary">
            <CardContent className="p-8 flex flex-col items-center gap-6">
              ...
              <p className="text-xs text-muted-foreground text-center">
                Sua senha está salva. Você pode fechar o navegador e voltar
                depois.
              </p>
            </CardContent>
          </Card>
